...
instance MonadTrans IdentityT where
  lift    = IdentityT
  mt      = MT
  unlift  = \k -> IdentityT (k (\tm -> runIdentityT tm >>= return . Identity) >>= return . runIdentity) 

instance MonadWriter w m => MonadWriter w (IdentityT m) where
  tell        = lift . tell
  listen  tm  = IdentityT $ listen $ runIdentityT tm 
  pass    tm  = IdentityT $ pass   $ runIdentityT tm
  

instance MonadState w m => MonadState w (IdentityT m) where
  put = lift . put
  get = lift get
